...
$app_stats = ['total' => 0, 'pending' => 0, 'selected' => 0, 'rejected' => 0, 'hired' => 0, 'avg_match' => 0];
$monthly_trends = [];
$dept_stats = [];
$job_performance = [];
$onboarding_stats = [];
...
try {
    ...
    // Monthly application trends (last 6 months) - PostgreSQL syntax
    $stmt = $pdo->query("SELECT 
    ...
    $stmt = $pdo->query("SELECT 
        ...
        LIMIT 10");
    $job_performance = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    $onboarding_stats = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    $training_stats = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Support ticket analytics (if table exists)
    $stmt = $pdo->query("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'support_tickets'");
    if ($stmt->fetchColumn() > 0) {
        $stmt = $pdo->query("SELECT 
            ...
            GROUP BY category
            ORDER BY COUNT(*) DESC");
        $support_stats = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
...
if (!$app_stats) $app_stats = ['total' => 0, 'pending' => 0, 'selected' => 0, 'rejected' => 0, 'hired' => 0, 'avg_match' => 0];
if (!$monthly_trends) $monthly_trends = [];
if (!$dept_stats) $dept_stats = [];
if (!$job_performance) $job_performance = [];
if (!$onboarding_stats) $onboarding_stats = [];
if (!$training_stats) $training_stats = [];
if (!$support_stats) $support_stats = [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="main-content">
        ...
        <div class="content-area">
            ...
            <div class="dashboard-grid">
                <div class="stat-card">
                    <div class="stat-number"><?php echo number_format($app_stats['total'] ?? 0); ?></div>
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [
                        ...
                        <?php echo $app_stats['selected'] ?? 0; ?>,
                        <?php echo $app_stats['hired'] ?? 0; ?>
                    ],
                    backgroundColor: [
                        'rgba(23, 162, 184, 0.8)',
                        'rgba(255, 193, 7, 0.8)',
            ...
            const reportContent = `
                <html>
                    ...
                    <body>
                        ...
                        <h2>Key Metrics</h2>
                        <div class="metric">
                            <h3><?php echo $app_stats['total'] ?? 0; ?></h3>
